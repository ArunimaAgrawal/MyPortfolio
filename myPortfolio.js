var TxtRotate = function (el, toRotate, period) {
        ...
        TxtRotate.prototype.tick = function () {
            ...
            var that = this;
            var delta = 300 - Math.random() * 100;    //300ms - random no
...
function toggleProjectDetails(projectId) {
        ...
        function toggleExperienceDetails(experienceId) {
            var experienceDetails = document.getElementById(experienceId + '-details');
            var allExperienceDetails = document.querySelectorAll('.experience-details');
            allExperienceDetails.forEach(function (item) {
                if (item.id !== experienceId + '-details') {
                    item.classList.remove('show-details');
                }
            });
            if (experienceDetails.classList.contains('show-details')) {
                experienceDetails.classList.remove('show-details');
            } else {
                experienceDetails.classList.add('show-details');
            }
        }
function downloadResume() {

    var resumeUrl = 'ArunimaAgrawal_Resume24.pdf';
